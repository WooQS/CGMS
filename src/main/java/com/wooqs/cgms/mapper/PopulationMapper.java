...
public interface PopulationMapper {
    ...
    @Insert("INSERT INTO population(communityId, name, gender, age, job, id_card, hometown, address) VALUES(#{communityId}, #{name}, #{gender}, #{age}, #{job}, #{idCard}, #{hometown}, #{address})")
    @Options(useGeneratedKeys = true, keyProperty = "populationId")
    void insert(Population population);

    @Update("UPDATE population SET community_id = #{communityId}, name = #{name}, gender = #{gender}, age = #{age}, job = #{job}, id_card = #{idCard}, hometown = #{hometown}, address = #{address} WHERE populationId = #{populationId}")
